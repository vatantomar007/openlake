...
"""Per-operation telemetry for the OpenLake connector.

One row per store RPC (save_exists, save_put, load_get) with duration,
key/byte counts, status, and failed-key count. Surfaced to the logger via
KVConnectorStats and to Prometheus via OpenLakePromMetrics.
"""
...
def _nearest_rank_percentile(values: list[float], percentile: float) -> float:
    ...
    rank = max(0, min(len(ordered) - 1, int(percentile * len(ordered) - 1e-12)))
    return ordered[rank]


@dataclass
class OpenLakeConnectorStats(KVConnectorStats):
    """Serializable store-communication telemetry."""
    ...
    def reduce(self) -> dict[str, int | float]:
        ...
        for operation, records in sorted(self.data.items()):
            ...
            reduced[f"{operation}_total_keys"] = sum(
                int(r["num_keys"]) for r in records)
            reduced[f"{operation}_total_bytes"] = sum(
                int(r["num_bytes"]) for r in records)
            reduced[f"{operation}_failed_keys"] = sum(
                int(r["num_failed_keys"]) for r in records)
...
class OpenLakePromMetrics(KVConnectorPromMetrics):
    """Prometheus metrics for store communication."""
    ...
    def __init__(
        ...
        self._histogram_operation_time = self._histogram_cls(
            name="vllm:openlake_operation_time_seconds",
            documentation="Histogram of OpenLake store communication time.",
            buckets=[
                1e-3, 5e-3, 1e-2, 5e-2, 1e-1, 2e-1, 3e-1, 4e-1, 5e-1,
                7.5e-1, 1.0, 1.5, 2.0, 3.0, 4.0,
            ],
            labelnames=metric_labelnames,
        )
        self._counter_operation_calls = self._counter_cls(
            name="vllm:openlake_operation_total",
            documentation="Number of OpenLake store communication operations.",
            labelnames=metric_labelnames,
        )
        self._counter_operation_keys = self._counter_cls(
            name="vllm:openlake_operation_keys_total",
            documentation="Number of OpenLake store keys touched by operations.",
            labelnames=metric_labelnames,
        )
        self._counter_operation_bytes = self._counter_cls(
            name="vllm:openlake_operation_bytes_total",
            documentation="Number of bytes transferred by OpenLake store operations.",
            labelnames=metric_labelnames,
        )
        self._counter_failed_keys = self._counter_cls(
            name="vllm:openlake_operation_failed_keys_total",
            documentation="Number of OpenLake store keys that failed in operations.",
            labelnames=metric_labelnames,
        )

    def _get_metrics(self, engine_idx: int, operation: str,
                     status: str) -> dict[str, PromMetric]:
